...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, args):
        ...
        print("开始读取物品曝光字典")
        t1=time.time()
        self.sorted_items=self.get_item_exposure()
        print(len(self.sorted_items))
        print(f"曝光字典读取完毕,用时：{time.time()-t1}s")
    ...
    def get_item_exposure(self):
        item_expo_dict_path = os.environ.get('USER_CACHE_PATH') + "/rematch_item_expo_dict_1.pkl"
        if os.path.isfile(item_expo_dict_path):
            with open(item_expo_dict_path, "rb") as f:
                item_expo_dict = pickle.load(f)
        else:
            item_expo_dict = {}
            for uid in range(len(self.seq_offsets)):
                u_data = self._load_user_data(uid)
                for record_tuple in u_data:
                    u, i, user_feat, item_feat, action_type, _ = record_tuple
                    if i and item_feat:
                        item_expo_dict[i] = item_expo_dict.get(i, 0) + 1
            item_expo_dict = sorted(
                item_expo_dict.items(),  
                key=lambda x: x[1],     
                reverse=True            
            )
            item_expo_dict = {item_id: count for item_id, count in item_expo_dict}
            with open(item_expo_dict_path, "wb") as f:
                pickle.dump(item_expo_dict, f)
        sorted_items = list(item_expo_dict.keys())
        return sorted_items
        
    def _cold_hot_neq(self, l, r, s):
        """
        生成一个不在序列s中的随机整数, 基于物品曝光次数分层采样
        30%概率选择曝光前20%的物品，10%概率选择中间40%，60%概率选择后40%
        Args:
            s: 序列（需排除的物品ID集合）
        Returns:
            t: 不在序列s中的随机整数
        ...
        if n == 0:
            raise ValueError("item_expo_dict为空，无法进行采样")
        top20 = int(n * 0.2)  
        mid40_start = top20
        mid40_end = int(n * 0.6)  
        ...
        if rand_p < 0.3:
            # 30%概率：选择前20%的物品
            if top20 == 0:
                layer_items = sorted_items
            else:
                l=0
                r=top20
        elif rand_p < 0.4:  # 0.3+0.1=0.4
            l=mid40_start
            r=mid40_end
        else:
            l=mid40_end
            r=end-1
        idx=self._random_neq(l,r,s)
        if idx >= len(self.sorted_items) or idx < 0:
            t = self._random_neq(l, r, s)
            
        else:
            t = self.sorted_items[idx]
        return t
    ...
    def __getitem__(self, uid):
        ...
        for record_tuple in reversed(ext_user_sequence[:-1]):
            ...
            if next_type == 1 and next_i != 0:
                ...
                neg_id = self._cold_hot_neq(1, self.itemnum + 1, ts)
                neg[idx] = neg_id
    ...
    def feat_list_to_tensor(self, feat_list):
        ...
        # 确保所有必要的属性都已初始化
        if not hasattr(self, 'ITEM_ARRAY_FEAT'):
            self.feature_default_value, self.feature_types, self.feat_statistics = self._init_feat_info()
            self._init_feat_info(self.feat_statistics, self.feature_types)
...
class MyTestDataset(MyDataset):
    ...
    def __getitem__(self, uid):
        ...
        # Convert features to tensors
        # 最后置1
        nxt_act = self.user_action_type[user_id]+1
        next_action_type[-1] = nxt_act
        seq_feat_tensor = self.feat_list_to_tensor(seq_feat)

        return seq, token_type,action_type,next_action_type, seq_feat_tensor, user_id
    ...
    def collate_fn(batch):
        ...
        Returns:
            ...
            user_id: user_id, str
        """
        seq, token_type,action_type,next_action_type, seq_feat, user_id = zip(*batch)
        seq = torch.from_numpy(np.array(seq))
        token_type = torch.from_numpy(np.array(token_type))
        action_type = torch.from_numpy(np.array(action_type))
        next_action_type = torch.from_numpy(np.array(next_action_type))
        ...
        return seq, token_type,action_type,next_action_type, seq_feat, user_id
